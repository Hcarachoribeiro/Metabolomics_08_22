...
DF <- read.table("../Data/MetabMSDIAL_Normalized_ver2_logT_imputed_RSD30_formatted_IMPORTANT.csv", header =T, sep = ',', stringsAsFactors = FALSE)

Metabs <- DF[,1]; DF = data.matrix(DF[,-1]); rownames(DF) <- Metabs;
Samp.nom = colnames(DF); ## LHS of the mapping file

print('Note CN3 samples are removed!')
Samp.nom = Samp.nom[-which(colnames(DF) %in% 'D01')];
DF = DF[,-which(colnames(DF) %in% 'D01')];

Grps = c( rep('Bipolar',1,length(grep('B',colnames(DF)))),
          rep('HC',1,length(grep('D',colnames(DF)))) );
          
print('Assigning groups to the column of the data.frame (DF)')

colnames(DF) <- Grps;  

print('Plot the distribution of the data')
plot.new()
hist(DF, col = 'lightblue', main = 'distribution of data')
...
pdf(paste0("../Results/Metabolite_Plots/PCA_metabolomics_no_QCs_knownOnly.pdf"), width = 14, height = 12, paper = 'special')
oripar=par()$mar
...
DF = apply(DF,2,function(x) tapply(x,rownames(DF),sum)) ## ADD different "TMS" derivatives
...
pdf(paste0("../Results/Metabolite_Plots/PLSDA_metabolomics_no_QCs_knownOnly_norm.pdf"), width = 14, height = 12, paper = 'special')
oripar=par()$mar
par(xpd=F, mar=oripar+c(2,2,0,6))
...
pdf(paste0("../Results/Metabolite_Plots/PLSDA_metabolomics_VIP_variable_selection.pdf"), width = 10, height = 8, paper = 'special')
 oripar=par()$mar
 par(xpd=F, mar=oripar+c(2,20,0,2));
 bh = barplot(vip1, horiz = T, col = cols , las=2, xlim=c(0,2), width = 0.1, space = 0.3, xlab = 'VIPs', main = 'PLSDA:VIPs', las=1, cex.names = 1, cex.axis = 1, border = 'black' );
...
save(Metab,file="../Data/Metab.rda")
save(Samp.nom,file="../Data/Samp.nom.rda")
